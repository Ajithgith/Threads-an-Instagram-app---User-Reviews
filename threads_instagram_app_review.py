...
def main():
    ...
    if rating_btt:
        ...
        print(rating_class)
        if rating_class == [1]:
            st.error('**Unacceptable**', icon='☹️')
        elif rating_class == [2]:
            st.warning('**Needs Improvement**', icon='😐')
        elif rating_class == [3]:
            st.success('**Meets Expectations**', icon='🙂')
        elif rating_class == [4]:
            st.success('**Exceeds Expectations**', icon='😀')
        else:
            st.success('**Outstanding**', icon='🤩')
